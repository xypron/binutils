...
riscv_fill_vregset (struct regcache *regcache, void *buf)
{
  ...
  /* Since vxsat and equivalent bits in vcsr are aliases (and same for vxrm),
   * we have a dilemma. For this gdb -> gdbserver topology,
   * if the aliased pairs have values that disagree, then
   ...
   * including vxsat, vxrm, and vcsr.
   * We have to impose some kind of rule for predictable resolution to resolve
   * any inconsistency. For now, let's say that vxsat and vxrm take precedence,
   * and those values will be applied to the corresponding fields in vcsr.
...
riscv_store_vregset (struct regcache *regcache, const void *buf)
{
  ...
  supply_register_by_name (regcache, "vcsr", regbuf);
  /* lso store offga non-byte-wise copy of vcsr,
   *  to derive values for vxsat and vxrm.  */
  vcsr = *(uint64_t*)regbuf;
  /* vlenb isn't part of vstate, but we have already inferred its
   * value by running code on this
