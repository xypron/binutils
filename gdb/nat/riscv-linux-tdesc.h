...
/* Some branches and/or commits of linux kernel named this
 * "struct __riscv_v_state",
 * and later it was changed to "struct __riscv_v_ext_state",
 * so using a macro to stand-in for that struct type to make
 * it easier to modify in a single place, if compiling against
 * one of those older Linux kernel commits.  */
...
struct __riscv_vregs
{
  ...
  /* Data will arrive packed, VLENB bytes per element, not necessarily
   *  RISCV_MAX_VLENB bytes per element.  */
};
