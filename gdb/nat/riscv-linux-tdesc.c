...
{
  /* This will gets us back to caller deeper in the call stack, with an
   * indication that an illegal instruction condition was encountered.   */
...
safe_read_vlenb ()
{
  /* Surrounding the attempt here to read VLENB CSR to have a signal handler
   * set up to trap illegal instruction condition (SIGILL), and if a trap
   * happens during this call,get control back within this function and
  ...
  if (SIGSETJMP (sigill_guard_jmp_buf, 1) == 0)
    ...
    {
      /* Must've generated an illegal instruction condition; we'll figure
       * this means no vector unit is present.  */
      vlenb = 0;
    }
  ...
  if (sysresult == 0)
	...
	{
	  perror
	    ("Error re-installing original SIGILL handler in "
	      "safe_read_vlenb()");
	}
