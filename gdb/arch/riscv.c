...
#ifdef GDBSERVER
/* Work around issue where trying to include riscv-tdep.h (to get access to
 * canonical RISCV_V0_REGNUM declaration
...
/* Usually, these target_desc instances are static for an architecture, and
 * expressable in XML format, but this is a special case where length of
 * a RISC-V vector register is not architecturally fixed to a constant
 * (the maximuim width is a defined constant,
                                             ...
                                             features)
{
  ...
  /* If VLENB is present (which we know it is present if execution reaches
   * this function),then we know by definition
   *  that it is at least 4 bytes wide.  */
  ...
  /* Need VLENB value checks for element chunks larger than 4 byte.  */
  ...
  /* QEMU and OpenOCD include the quads width in their target descriptions,
   * so we're following that precedent, even if it's not particularly useful
   * in practice, yet.  */
  ...
  /* Again, we know vlenb is >= 4, so no if guards needed for words/shorts/
   * bytes.  */
  ...
  tdesc_create_reg (feature, "vstart", regnum++, 1, NULL,
                     features.xlen * 8, "int");
  tdesc_create_reg (feature, "vxsat", regnum++, 1, NULL,
                     features.xlen * 8, "int");
  tdesc_create_reg (feature, "vxrm", regnum++, 1, NULL,
                     features.xlen * 8, "int");
  tdesc_create_reg (feature, "vcsr", regnum++, 1, NULL,
                     features.xlen * 8, "int");
  tdesc_create_reg (feature, "vl", regnum++, 1, NULL,
                     features.xlen * 8, "int");
  tdesc_create_reg (feature, "vtype", regnum++, 1, NULL,
                     features.xlen * 8, "int");
  tdesc_create_reg (feature, "vlenb", regnum++, 1, NULL,
                     features.xlen * 8, "int");
