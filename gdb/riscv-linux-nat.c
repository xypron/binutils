...
{
  { 1, RISCV_CSR_VSTART_REGNUM, MEMBER_SIZE(struct __riscv_vregs,
		  vstate.vstart) },
  { 1, RISCV_CSR_VL_REGNUM, MEMBER_SIZE(struct __riscv_vregs, vstate.vl) },
  { 1, RISCV_CSR_VTYPE_REGNUM, MEMBER_SIZE(struct __riscv_vregs,
		  vstate.vtype) },
...
/* Other RISC-V regsets use riscv_supply_regset here; not sure
 * that'd be correct for this case.  */
  regcache_collect_regset
};


/* Supply RISC-V vector register values (including inferred CSRs) to the
 * GDB regcache.  */
		       ...
		       const struct __riscv_vregs *vregs, int regnum)
{
  ...
  regcache_supply_regset (&riscv_linux_vregset, regcache, regnum, vregs,
		  sizeof(*vregs));

  if (regnum == -1 || regnum == RISCV_CSR_VLENB_REGNUM)
    {
      /* We already have a local copy above, use that
       * (widened for XLEN padding).  */
  ...
  if (regnum == -1 || regnum == RISCV_CSR_VXSAT_REGNUM)
    {
      /*  This CSR is not part of vregs->vstate literally, but we can infer
       *   a value from vcsr.  */
      uint64_t vxsat =
	      ((vregs->vstate.vcsr >> VCSR_POS_VXSAT) & VCSR_MASK_VXSAT);
      buf = (gdb_byte *) & vxsat;
  ...
  if (regnum == -1 || regnum == RISCV_CSR_VXRM_REGNUM)
    {
      /*  This CSR is not part of vregs->vstate literally, but we
       *  can infer a value from vcsr.  */
...
/* Collect RISC-V vector register values (including inferred CSRs)
 *  from the GDB regcache.  */
static void
fill_vregset (const struct regcache *regcache, struct __riscv_vregs *vregs,
	      int regnum)
{
  regcache_collect_regset (&riscv_linux_vregset, regcache, regnum, vregs,
		  sizeof(*vregs));
  ...
  if (regnum == -1 || regnum == RISCV_CSR_VCSR_REGNUM ||
      ...
      || regnum == RISCV_CSR_VXRM_REGNUM)
    {
      ...
      if ( ! (regnum == -1 || regnum == RISCV_CSR_VCSR_REGNUM) )
	{
	  /* We don't already have the VCSR value, from the
	   *  earlier regcache_collect_regset call, so let's get it now.  */
	  regcache_collect_regset (&riscv_linux_vregset, regcache,
			  RISCV_CSR_VCSR_REGNUM, vregs, sizeof(*vregs));
	}
      ...
      if (regnum == RISCV_CSR_VXSAT_REGNUM)
	{
	  ...
	  vregs->vstate.vcsr &= ~((uint64_t) VCSR_MASK_VXSAT <<
			  VCSR_POS_VXSAT);
	  vregs->vstate.vcsr |= ((vxsat_from_regcache & VCSR_MASK_VXSAT) <<
			  VCSR_POS_VXSAT);
	}
      ...
      if (regnum == RISCV_CSR_VXRM_REGNUM)
	{
	  ...
	  gdb_byte *buf = (gdb_byte *) &vxrm_from_regcache;
  ...
  /* VLENB register is not writable, so that's why nothing is collected
   * here for that register.  */
