...
/* Magic number written to the head.magic field of struct __sc_riscv_v_state
 * that kernel places in the reserved area of struct sigcontext.
 * Comes from <asm/sigcontext.h>.   */
...
/* Read RVV magic and size fields from address REGS_BASE and return true if a
 * valid RISC-V vector context header is found, otherwise return false.
 * THIS_FRAME is used for the architecture and byte-order when
 * reading memory. VLEN and XLEN are the v-register and x-register
 * sizes (in bytes) and are used for validation.  */
					  ...
					  CORE_ADDR regs_base)
{
  ...
  /* If vector information is available, then we should see this
     ...
     __u32 size;
     (size of struct __sc_riscv_v_state + vector register data size (32*VLENB))
     } head;   */
  ...
  if (!info_good)
    {
      /* Not an error, because kernels can be configured without CONFIG_VECTOR,
       * but worth noting if frame debug setting is turned on.  */
      frame_debug_printf
	("Did not find RISC-V vector information in ucontext"
	 "(kernel not built with CONFIG_VECTOR?)");
  ...
  if (frame_debug)
    {
      ...
      frame_debug_printf
	("Located RISC-V vector information in signal frame ucontext"
	 "(info size %u)",
	 rvv_size);
	...
	{
          warning (_("Size in RISC-V vector information header in ucontext"
				  "(%u) differs from expected size (%u)."),
             rvv_size, expected_rvv_size);

          /* Additional debug details if frame_debug is on.  */
	  frame_debug_printf ("Detailed size mismatch: expected %u based on"
			  "VLEN=%d and XLEN=%d",
				  ...
				  CORE_ADDR regs_base, int xlen, int vlen)
{
  ...
  /* vstart, vl, vtype, vcsr, and datap are XLEN sized fields (unsigned long)
   * from this point.  */
  vfieldidx = 0;
  ...
  /* For the datap member, there is one level of memory indirection
   * to get the address of the block of values for v0..v31.  */
  vfieldidx++;
...
/* offset of struct mcontext's __reserved field,
